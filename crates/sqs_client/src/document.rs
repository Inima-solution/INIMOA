...
impl SQS {
    /// Bulk enqueues deleted-document candidates with their exact soft-delete
    /// timestamp token. Legacy ownerless messages intentionally remain unchanged.
    #[tracing::instrument(skip(self))]
    pub async fn bulk_enqueue_document_purge_candidates(
        &self,
        candidates: Vec<(String, String)>,
    ) -> anyhow::Result<()> {
        if let Some(document_delete_queue) = &self.document_delete_queue {
            return enqueue_delete_document::bulk_enqueue_document_purge_candidates(
                &self.inner,
