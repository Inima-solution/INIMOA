...
use super::{
    ...
    task_dependency_queries, task_dependency_read_queries, task_dependency_relation_queries,
    task_property_queries, task_status_transition_queries, task_subtask_progress_queries,
};
use crate::domain::model::{
    ...
    TaskDependencyMutationOutcome, TaskDependencyReadiness, TaskDependencyRelationsSnapshot,
...
impl PropertiesRepo for PropertiesPgRepo {
    ...
    #[tracing::instrument(skip(self, task_ids), err)]
    async fn get_task_dependency_relations(
        &self,
        task_ids: &[Uuid],
    ) -> Result<Option<Vec<TaskDependencyRelationsSnapshot>>, Self::Err> {
        task_dependency_relation_queries::get_task_dependency_relations(&self.pool, task_ids).await
    }
