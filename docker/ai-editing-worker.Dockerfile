...
RUN bun install --frozen-lockfile --filter ai-editing-worker --ignore-scripts \
	&& node -e 'const fs=require("fs"); const workerd=require("workerd/package.json"); const wrangler=require("wrangler/package.json"); if (wrangler.version!=="4.110.0" || workerd.version!=="1.20260708.1") process.exit(1); const targets={arm64:["@cloudflare/workerd-linux-arm64/bin/workerd",183],x64:["@cloudflare/workerd-linux-64/bin/workerd",62]}; const selected=targets[process.arch]; if (!selected) process.exit(1); const [modulePath,machine]=selected; const p=require.resolve(modulePath); const b=fs.readFileSync(p); if ((fs.statSync(p).mode&0o111)===0 || b[0]!==0x7f || b[1]!==0x45 || b[2]!==0x4c || b[3]!==0x46 || b.readUInt16LE(18)!==machine) process.exit(1); for (const candidate of ["@cloudflare/workerd-linux-arm64/bin/workerd","@cloudflare/workerd-linux-64/bin/workerd","@cloudflare/workerd-darwin-arm64/bin/workerd","@cloudflare/workerd-darwin-64/bin/workerd"]) { if (candidate===modulePath) continue; try { require.resolve(candidate); process.exit(1); } catch (error) { if (error.code!=="MODULE_NOT_FOUND") throw error; } }'
